document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadLogs() {
        ...
        if (uvuId.length === 8 && /^\d+$/.test(uvuId)) {
            try {
                const response = await fetch(`http://localhost:4000/logs?courseId=${courseId}&uvuId=${uvuId}`);
                if (!response.ok && response.status === 404) {
                    // Display message if no logs are found, but allow adding a new log
                    logsContainer.innerHTML = '<li class="list-group-item">No logs found for this UVU ID. You can start by adding one below!</li>';
                    uvuIdDisplay.textContent = `Student Logs for ${uvuId}`;
                    uvuIdDisplay.style.display = 'block';
                    textareaContainer.style.display = 'block';
                    submitButton.style.display = 'block';
                    submitButton.disabled = false; // Enable the submit button for adding logs
                } else {
    ...
    document.querySelector('form').addEventListener('submit', async (event) => {
        ...
        try {
            ...
            // Clear the "No logs found" message if it's present
            if (logsContainer.textContent.includes("No logs found for this UVU ID")) {
        ...
        `;
            logItem.addEventListener('click', () => {
                const logText = logItem.querySelector('pre');
                logText.style.display = logText.style.display === 'none' ? 'block' : 'none';
            });
            logsContainer.appendChild(logItem);

            // Clear the textarea and disable the button after submission
            textarea.value = '';
            submitButton.disabled = true;
        } catch (error) {
