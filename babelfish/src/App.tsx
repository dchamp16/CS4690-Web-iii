...
import { Mic, StopCircle, Volume2, Languages } from 'lucide-react';
...
function App() {
  ...
  const speakMessage = async (text: string) => {
    ...
    // Find the appropriate voice based on primary language
    const availableVoices = window.speechSynthesis.getVoices();
    let voice = availableVoices.find(v => v.lang === primaryLanguage);
  ...
  const sendMessage = (text: string) => {
    if (ws.current?.readyState === WebSocket.OPEN) {
      ws.current.send(JSON.stringify({
        text,
        username,
        sourceLanguage: primaryLanguage.startsWith('ja') ? 'ja' : primaryLanguage.startsWith('es') ? 'es' : 'en',
        targetLanguage: secondaryLanguage.startsWith('ja') ? 'ja' : secondaryLanguage.startsWith('es') ? 'es' : 'en',
      }));
    }
  };
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-blue-400 to-purple-500">
          <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-md">
            <h1 className="text-2xl font-semibold text-center mb-6 text-gray-700">BabelFish</h1>
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-600 mb-1">Primary Language</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium text-gray-600 mb-1">Secondary Language</label>
                <select
              ...
              >
                {primaryLanguage === 'ja-JP' ? '入る' : primaryLanguage === 'es-ES' ? 'Entrar' : 'Enter'}
              </button>
            </form>
          </div>
        </div>
        ...
        <header className="bg-white shadow-sm p-4">
          <div className="max-w-3xl mx-auto flex justify-between items-center">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Primary Language</label>
                <select
                    value={primaryLanguage}
                    onChange={(e) => setPrimaryLanguage(e.target.value)}
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Secondary Language</label>
                <select
                    value={secondaryLanguage}
                    onChange={(e) => setSecondaryLanguage(e.target.value)}
                    className="text-sm border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-blue-400"
                ...
                >
                  ...
                  <div
                      className={`max-w-[80%] rounded-lg p-3 ${
                          message.sender === username ? 'bg-blue-600 text-white' : 'bg-white text-gray-800'
                      }`}
                  >
          ...
          <div className="max-w-3xl mx-auto flex items-center space-x-4">
            <button
                ...
                className={`p-2 rounded-full ${
                    isRecording ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-blue-100 text-blue-600 hover:bg-blue-200'
                }`}
            >
            ...
            <button
                onClick={() => {
                  ...
                  if (lastMessage) {
                    speakMessage(lastMessage.translation);
                  }
                }}
